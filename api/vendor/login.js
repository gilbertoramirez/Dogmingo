...
module.exports = async function handler(req, res) {
  ...
  if (email === 'admin' && password === (process.env.ADMIN_PASSWORD || '')) {
    ...
    return res.status(200).json({ ok: true, token, vendor: { nombre: 'Administrador', stand_num: 0, es_admin: true } });
  }
  ...
  try {
    ...
    return res.status(200).json({ ok: true, token, vendor: { nombre: vendor.nombre, stand_num: vendor.stand_num, es_admin: vendor.es_admin } });
  } catch (err) {
